function [] = makeSupplementFigureS3B(sensorsToAverage, summaryMetric, opt)
% Function to plot Supplement Figure S3B, showing average variance explained 
...
if opt.saveFig
    ...
    fprintf('\n(%s): Saving Supplement Figure S3B in %s\n',mfilename, saveDir);
  
    figurewrite(fullfile(saveDir, sprintf('SuppFigureS3B_IndividualSubjects_varySizeSummary%s_%s_%s', opt.fNamePostFix, sensorsToAverage, summaryMetric)),[],0,'.',1);
    print(fH1, fullfile(saveDir, sprintf('SuppFigureS3B_IndividualSubjects_varySizeSummary%s_%s_%s', opt.fNamePostFix, sensorsToAverage, summaryMetric)), '-dpng');
end
