...
for s = 1:length(subjects)
    ...
    load(fullfile(dirPth.model.saveDataPth, opt.subfolder, 'pred_resp', 'splitHalfAmpReliability1000.mat'));
    allSubjectData(s,:) = splitHalfAmpCorrelation;
    
    % Plot split half amplitude reliability
    subplot(2,5,s);
    ttl = sprintf('S%d', s);
    megPlotMap(allSubjectData(s,:),[0 .8],fH1, 'hot', ...
    ...
    if opt.saveFig
        figure(fH1)
        print(fH1, fullfile(saveDir, sprintf('Figure4_S%d_SSVEFReliabilityCorrelation_All_%s',  opt.fNamePostFix,interpMethod)), '-dpng');
        figurewrite(fullfile(saveDir, sprintf('Figure4_S%d_SSVEFReliabilityCorrelation_All_%s',  opt.fNamePostFix,interpMethod)),[],0,'.',1);
        fprintf('(%s) Saving figure 3C - Individual Subject SSVEF reliability in %s\n',mfilename, saveDir);
    end

% Plot split half amplitude reliability
figure(fH2); clf;
megPlotMap(nanmean(allSubjectData,1),[0 0.8],fH2, 'hot', ...
