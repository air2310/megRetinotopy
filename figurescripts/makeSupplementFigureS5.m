function makeSupplementFigureS5(subjectToPlot, plotAverage)
% Function to make Supplemental Figure S5 of manuscript, showing the effect
...
for s = subjectToPlot
    ...
    if opt.saveFig
        ...
        fprintf('\n(%s): Saving Supplemental Figure S5 in %s\n',mfilename, saveDir);
        figure(fH1);
        figurewrite(fullfile(saveDir, sprintf('SupplFigureS5_%s_varySizeMeshes%s', dirPth.subjID, opt.fNamePostFix)),[],[1 300],'.',1);
        %     figurewrite(fullfile(saveDir, sprintf('SupplFigureS5_%s_varySizeMeshes%s', dirPth.subjID, opt.fNamePostFix)),[],0,'.',1);
