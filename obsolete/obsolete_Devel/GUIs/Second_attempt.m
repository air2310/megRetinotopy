function varargout = Second_attempt(varargin)
% SECOND_ATTEMPT MATLAB code for Second_attempt.fig
%      SECOND_ATTEMPT, by itself, creates a new SECOND_ATTEMPT or raises the existing
%      singleton*.
%
%      H = SECOND_ATTEMPT returns the handle to a new SECOND_ATTEMPT or the handle to
%      the existing singleton*.
%
%      SECOND_ATTEMPT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SECOND_ATTEMPT.M with the given input arguments.
%
%      SECOND_ATTEMPT('Property','Value',...) creates a new SECOND_ATTEMPT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Second_attempt_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Second_attempt_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Second_attempt

% Last Modified by GUIDE v2.5 22-May-2017 12:05:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Second_attempt_OpeningFcn, ...
    'gui_OutputFcn',  @Second_attempt_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Second_attempt is made visible.
function Second_attempt_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Second_attempt (see VARARGIN)
...
% Choose default command line output for Second_attempt
...
% UIWAIT makes Second_attempt wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Second_attempt_OutputFcn(hObject, eventdata, handles)
