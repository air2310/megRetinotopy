function bs_msh = mprfSession_visualize_data_on_brainstorm_surface(bs_msh)
...
if ~exist('bs_msh','var') || isempty(bs_msh)
    ...
    bs_hm = mprf__get_file('bs_headmodel');
    load(fullfile(bs_hm.fpath, bs_hm.fname.name));
    
    [~, surf_name] = fileparts(bs_model.SurfaceFile);
    
    bs_msh = mprfMeshFromBrainstorm(fullfile(mprf__get_directory('main_dir'), mprf__get_directory('bs_anat'),surf_name));
