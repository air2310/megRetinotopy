...
%     meg_ori_idx = 1:5;
    ...
    all_triggers = [10 20 all_orientations+1]; % 10 = blank, 20 = blink, 1-9 are orientations - 1 * 45 = orientation in degrees
    ...
    blink_blank_frame = (blink_time_meg + blank_time_meg) .* flicker_freq;
    
    seq_meg(logical(blink_idx_all)) = 2;
    seq_meg = [seq_meg(end - blink_blank_frame+1 : end) seq_meg];
    n_frames_meg = length(seq_meg);
...
if ~isempty(mri_save_path)
    ...
    save(fullfile(mri_save_path,'MRI_grid'),'grid');
    clear grid
end
