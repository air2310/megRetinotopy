...
for n_surf = 1:length(surfaces_to_load)
    for n_hs = 1:length(hs_to_load)
        ...
        for n_prfpar = 1:length(par_names)
            
            cur_par_name = par_names{n_prfpar};
...
if opt.roimrvToFS == 1
    for n_surf = 1:length(surfaces_to_load)
        for n_hs = 1:length(hs_to_load)
            if strfind(hs_to_load{n_hs},'lh')
                roi_path = fullfile(roi_dir);
                tmp = dir(fullfile(roi_path,'L*.mat'));
                roi_names = {tmp.name};
                hs_tag = 100;
            elseif strfind(hs_to_load{n_hs}, 'rh')
                roi_path = fullfile(roi_dir);
            ...
            % Loop over the ROIS
            for n_rois = 1:length(roi_names)
                [~,r_name] = fileparts(roi_names{n_rois});
                cnt = cnt+1;
                
                % Load the current ROI
                load(fullfile(roi_path{1},roi_names{n_rois}))
                ...
                roi_vert_inds_all{n_rois} = roiVertInds;
                roi_vert_inds_all2{n_rois} = roiVertInds2;
            ...
            for n_rois = 1:length(roi_names)
                cnt = cnt+1;
                roi_tag = mprf__get_roi_tags(roi_names{n_rois});
                
                tag_to_idx.(roi_tag) = cnt;
                
                [~,roi_name] = fileparts(roi_names{n_rois});
                tmp = nan(1,length(mrv_msh.vertices));
                
                if numel(pruned_roi_inds{n_rois})
                    
                    all_rois(pruned_roi_inds{n_rois}) = cnt;
                    tmp(pruned_roi_inds{n_rois}) = cnt;
