...
# you want to run.
matlab -nodisplay -r "addpath(genpath(brainstorm3)); addpath(genpath(meg_utils)); addpath(genpath(noisepoolPCADenoise)); addpath(genpath(vistasoft)); addpath(fieldtrip); ft_defaults; cd('megRetinotopy'); mprf_addPaths; subjID='wlsubj058'; run('runModelMain$SLURM_ARRAY_TASK_ID(subjID)); exit()"
