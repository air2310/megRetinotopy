...
for i = 1:numel(preproc_paths)
    preproc_path = fullfile(session_path, session_name, 'raw', 'derivatives', preproc_paths{i});
...
d = dir(fullfile(preproc_path, sprintf('sub-%s', sub_name), sprintf('ses-%s',bids_sess), '*preproc.nii.gz'));
for ii = 1:length(d)
    epi_files{ii} = fullfile(preproc_path, sprintf('sub-%s', sub_name), sprintf('ses-%s',bids_sess), d(ii).name);
end
fprintf('Found %d preprocessed EPI files...\n', numel(epi_files));

% Inplane (mean of unwarped, merged, distortion scan)
ip = fullfile(preproc_path, sprintf('sub-%s', sub_name), sprintf('ses-%s',bids_sess), 'distortion_merged_corrected_mean.nii.gz');
if ~exist(ip, 'file'), error('Inplane file %s not found', ip); end

% alignment file
align_file = fullfile(preproc_path, sprintf('sub-%s', sub_name), sprintf('ses-%s',bids_sess),  'distort2anat_tkreg.dat');
