...
param_dir = '/Users/bpklein/Desktop/';
par_files.name = '20170406T083126.mat';
file_idx = 1;
...
for n = 1:length(file_idx)
    ...
    [flip_time, stim_time, time0] = mprfGetStimAndFlipTime(stimulus,response, time0);
...
for n = 1:length(stim_flip_time)
    ...
    all_added_delay(n) = added_delay;
    all_accum_delay(n) = accum_delay;
    
    if added_delay > 0;%floor(1/params.display.frameRate * 1000)/2;
